...
Add one shared helper at module level, immediately after `_label_block`. It takes `cfg` as an explicit argument, so it does not need to be a closure. Immediately above it, add a module-level constant for the media column's percentage width - `_bookmark_media_text` reads both the block's `mediaWidth` attribute and the `grid-template-columns` style it implies from this one place, so the two can't drift apart:
