...
def merge_config(user_config=None, warn=None):
    ...
    names. Unknown type names warn and are dropped.
    """
    ...
    if "background" in user_config:
        merged["background"] = user_config["background"]
    if "padding" in user_config:
        merged["padding"] = user_config["padding"]

    for name, overrides in (user_config.get("types") or {}).items():
        key = str(name).lower()
        if key not in merged["types"]:
            warn(f"unknown callout type in config, ignored: {name}")
            continue
        merged["types"][key].update(overrides)
...
def _group_attrs(type_name, cfg):
    ...
    style = {
        "border": {"left": {"color": color_attr(accent), "width": "4px"}},
        "spacing": {
            "padding": {
                "top": padding,
                "right": padding,
...
In `tests/test_gutenberg.py`, the `TestAdmonitions` class asserts the old markup. Replace the class name with `TestCalloutsIntegration` and update every assertion: `is-admonition` becomes `is-callout`, `is-admonition-<type>` becomes `is-callout-<type>`, and `"Note</p>"`-style assertions become `"Note</strong>"`. Keep `test_regular_blockquote_unchanged` exactly as it is - a plain blockquote must still produce `wp:quote`.
...
- [ ] **Step 5: Verify the front end**

Run: `curl -s https://dashpadd.com/callouts-demo/ | grep -c "is-callout"`
Expected: a non-zero count.

Then check the three things the spec could not settle in advance:

```bash
curl -s https://dashpadd.com/callouts-demo/ > /tmp/demo.html
grep -o 'border-left-color:[^;"]*' /tmp/demo.html | head    # split border survived?
grep -o '<summary>.\{0,80\}' /tmp/demo.html | head          # h3 inside summary survived?
grep -c '<svg' /tmp/demo.html                               # icons survived kses?
```

- [ ] **Step 6: Record what the demo settled**

Append a short "Verified on publish" section to `docs/superpowers/specs/2026-07-31-callouts-design.md` recording the actual answer to each of the three checks, replacing the "To verify during implementation" predictions with what happened. If the `<h3>` inside `<summary>` or the split border did not survive, apply the fallback named in the spec and re-publish before writing this up.
...
| To verify during implementation | 8 (step 5) |
| Demo                     | 8     |
...
**Known deviation from the spec:** the spec's padding used `var:preset|spacing|small`. This plan uses a literal rem value, configurable via `callouts.padding`, because a theme lacking the spacing slug would silently lose all padding and leave text flush against the background edge. Colour behaviour is unchanged.
